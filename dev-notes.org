...
#+BEGIN_SRC elisp
(defun sallet-filter-NAME (pattern candidates indices)
...
*** Item filters
In turn, filters can use /item filters/ to match candidates and update
their index metadata.

It is important to realize that item filters are completely
...
update, that's it.  Filters using item filters can arbitrarily process
...
Item filters have this interface:

#+BEGIN_SRC elisp
(defun sallet-filter-item-NAME (candidate index pattern)
